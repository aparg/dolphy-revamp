...
export async function getProperties({
  ...
  queryArray.push("PropertyType ne 'Commercial'");
  ...
  if (propertySubTypes && propertySubTypes.length > 0) {
    if (propertySubTypes[0] == "Semi-Detached") {
      queryArray.push(`PropertySubType eq 'Semi-Detached '`);
    } else {
...
// Add the open house functions
export async function fetchOpenHouseFromMls() {
  const options = {
    method: "GET",
    headers: {
...
export async function fetchOpenHouse({ city = null }) {
  const options = {
    method: "GET",
    headers: {
      Authorization: process.env.BEARER_TOKEN_FOR_VOW,
  ...
  const url = `${baseUrl}OpenHouse?$top=500&$orderby=OpenHouseStartTime desc`;
  ...
  const cacheKey = city?.toLowerCase() || "ontario";
  ...
  for (let i = 0; i < value.length; i += batchSize) {
    ...
    if (city) {
      propertyFetchString = `${baseUrl}Property?$filter=contains(City,'${city}') and (${orString})`;
    } else {
      propertyFetchString = `${baseUrl}Property?$filter=(${orString})`;
    }
...
export async function getPropertiesCounts({
  city,
  propertyType,
  propertySubTypes,
  minBeds,
  ...
  try {
    ...
    return {
      total: data["@odata.count"],
      minPrice: Math.min(...data.value.map((property) => property.ListPrice)),
      maxPrice: Math.max(...data.value.map((property) => property.ListPrice)),
      averagePrice:
        data.value.reduce((sum, property) => sum + property.ListPrice, 0) /
        data.value.length,
    };
  } catch (error) {
    console.error("Error fetching properties:", error);
